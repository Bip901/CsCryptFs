// Ported from https://github.com/alexey-lapin/eme-java/blob/master/src/test/java/com/github/alexeylapin/eme/EMEImplTest.java
using System.Security.Cryptography;
using Xunit;

namespace CsCryptFs.Tests;

public sealed class EMEEngineTests
{
    private static readonly byte[] Buf9F2E =
    [
    ...
    private static readonly byte[] Buf0809 =
    [
    ...
    public void Encrypt()
    {
        using EMEEngine eme = new(new byte[32]);

        byte[] encrypted = eme.Encrypt(new byte[16], new byte[512]);

        Assert.Equal(Buf9F2E, encrypted);
    }
    ...
    public void EncryptDecrypt()
    {
        ...
        byte[] decrypted = eme.Decrypt(new byte[16], encrypted);

        Assert.Equal(new byte[512], decrypted);
    }
    ...
    [InlineData(8)]
    [InlineData(9)]
    public void EncryptDecryptRandom10Times(int _)
    {
        byte[] tweak = RandomNumberGenerator.GetBytes(16);
        byte[] input = RandomNumberGenerator.GetBytes(512);
    ...
    public void Encrypt100Times()
    {
        ...
        byte[] tweak = [0x3C, 0x9C, 0x27, 0x68, 0x38, 0xCC, 0x5E, 0x14, 0x11, 0xFC, 0xB8, 0xCF, 0x3D, 0xA1, 0xC0, 0xF3];
        ...
        using EMEEngine eme = new(key);
        for (int i = 0; i < 100; i++)
        {
            output = eme.Encrypt(tweak, output);
        }
    ...
    public void Decrypt100Times()
    {
        ...
        byte[] tweak = [0x5F, 0x29, 0x3E, 0x85, 0xF1, 0xEA, 0xA8, 0xEE, 0x23, 0x22, 0xF5, 0x42, 0x91, 0xBF, 0x05, 0x1E];
        ...
        for (int i = 0; i < 100; i++)
        {
            output = eme.Decrypt(tweak, output);
        }
